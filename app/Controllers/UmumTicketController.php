...
class UmumTicketController extends BaseController
{
    ...
    public function create()
    {
        ...
        // =====================================================
        // AMBIL PROFILE PENGGUNA DARI DATABASE
        // =====================================================
        ...
        $pemohon = [
            ...
            // PRIORITAS: data profile dari database (paling baru)
            // karena session user bisa saja masih menyimpan data lama
            // sebelum user mengedit profile.
            'nama' => $profile['name']
                ?? $user['nama']
                ?? $user['full_name']
                ?? 'Pengguna Umum',
...
public function saveDraft()
{
    ...
    $ticketNumber = 'ULT-UMU-' . strtoupper(
    ...
    $serviceRequestId = $serviceRequestModel->getInsertID();
    ...
    // ==========================================
    // SIMPAN DATA UNTUK HALAMAN DRAFT SUCCESS
    // ==========================================

    $draft = [
        'id'            => $serviceRequestId,
        'nomor_draft'   => $ticketNumber,
        'service_id'    => $serviceId,
    ...
    // ==========================================
    // REDIRECT DRAFT SUCCESS
    // ==========================================

    return redirect()->to(
        base_url('umum/ticket/draft-success')
    );
}
...
$builder->where('sr.deleted_at', null);
        ...
        // Hanya draft milik pengguna yang sedang login
        if (empty($userProfileId)) {
            return redirect()
                ->to(base_url('login'))
                ->with(
        ...
        foreach ($drafts as &$draft) {
            ...
            foreach ($uploaded as $file) {
                ...
                $uploadedRequirementIds[$file['requirement_id']] = true;
            }
            ...
            foreach ($requirements as $requirement) {
                ...
                if (
                    !isset(
                        $uploadedRequirementIds[$requirement['id']]
                    )
                ) {
                    $complete = false;
                    break;
    ...
    public function editDraft($id)
    {
        ...
        foreach ($files as $file) {
            ...
            $uploadedFiles[$file['requirement_id']] = $file;
        }


        return view(
            'umum/ticket/edit_draft',
            [
    ...
    public function updateDraft($id)
{
    ...
    public function draftSuccess()
    {
        ...
        return view('umum/ticket/draft_success', [
    ...
    public function success()
    {
        ...
        if (!$ticket) {
            return redirect()->to(
                base_url('umum/ticket/create')
            );
        }

        return view('umum/ticket/success', [
    ...
    public function history()
    {
        ...
        if (!$userProfile) {
            return redirect()
                ->to(base_url('umum/dashboard'))
                ->with('error', 'Data profil pengguna tidak ditemukan.');
        }


        // ==========================================
        // AMBIL TIKET MILIK PENGGUNA
        // ==========================================
    ...
    public function detail($id)
    {
        ...
        if (empty($userProfileId)) {
            return redirect()
                ->to(base_url('umum/ticket/history'))
                ->with('error', 'Data profil pengguna tidak ditemukan.');
        }
    ...
    public function reply($id)
    {
        ...
        return redirect()->to(
            base_url('umum/ticket/detail/' . $id)
        );
    }
