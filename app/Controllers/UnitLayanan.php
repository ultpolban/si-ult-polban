...
{ 
    ...
    { 
        ...
        $tiket['judul'] = $tiket['judul'] 
            ?? $tiket['nama_layanan'] 
            ?? 'Permohonan Legalisir Ijazah'; 
 
        $tiket['deskripsi'] = $tiket['deskripsi'] 
            ?? 'Tidak ada deskripsi tambahan.'; 
 
        $tiket['nama_layanan'] = $tiket['nama_layanan'] 
            ?? 'Legalisir Ijazah'; 
 
        $tiket['nama_kategori'] = $tiket['nama_kategori'] 
            ?? 'Legalisasi Dokumen'; 
 
        $tiket['nama_unit'] = $tiket['nama_unit'] 
            ?? 'Akademik'; 
 
        $tiket['status'] = $tiket['status_tiket'] 
            ?? $tiket['status_pengajuan'] 
            ?? $tiket['status'] 
            ?? 'Menunggu'; 
    ...
    { 
        ...
        $uploadResult = $this->simpanDokumenHasil( 
            $tiket['id'], 
            $files 
        ); 
    ...
    {
        return view('unit_layanan/profile', [
    ...
    { 
        ...
        if (empty($files)) { 
            return redirect() 
                ->back() 
                ->with('error', 'Silahkan pilih dokumen terlebih dahulu'); 
        } 
    ...
    { 
        ...
        $status = strtolower((string) ($tiket['status'] ?? '')); 
        ...
        return redirect() 
            ->to('/unit-layanan/detail/' . ($tiket['id'] ?? $id)) 
            ->with('success', 'Tiket berhasil dikirim ke pemohon.'); 
    } 
